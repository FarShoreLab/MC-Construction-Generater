...
def build(spec):
    ...
    obj['entrances']=[obj['entrance']]
    for facing,dx,dz in [('NORTH',0,-1),('EAST',1,0),('WEST',-1,0)]:
        choices=[p for p in cells if (p[0]+dx,p[1]+dz) not in cells]
        x,z=min(choices,key=lambda p:abs(p[0]-w/2)+abs(p[1]-d/2)+(p[1] if facing=='NORTH' else w-1-p[0] if facing=='EAST' else p[0])*4)
        obj['entrances'].append(dict(facing=facing,x=x,z=z))
    obj['components']=[dict(id='main',role='main',mask=obj['footprintMask'])]
...
def estate(size):
    ...
    obj=dict(id=ident,name=f'{size}格·{names[size]}',category='government',tags=['government','compound','estate'],
        ...
        archived=True,palette=palette,themeReplacements={},layers=[[''.join(row) for row in layer] for layer in grid])
    (OUT/f'{ident}.json').write_text(json.dumps(obj,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
