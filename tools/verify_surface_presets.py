"""Real HTTP checks for archived estates, road materials and farm construction."""
import argparse
import json
import urllib.error
...
def main():
    ...
    ap.add_argument('--base',default='http://127.0.0.1:18771')
    ...
    for material,voxel in [('dirt_path',16),('gravel',7),('cobblestone',8),('stone_bricks',9)]:
        ...
        assert data['config']['roadSurface']==material and len(data['presets'])==34
    ...
    for size in [48,64,96]:
        ...
        assert code==400 and 'ARCHIVED' in data['error'],data
        results.append({'case':f'archived_estate_{size}','status':'PASS'});print('PASS archived estate',size,flush=True)
    ...
    assert sum(p['presetId'].startswith('estate_') for p in data['plan']['plots'])==0
    ...
    assert code==400 and 'ARCHIVED' in data['error']
    results.append({'case':'archived-on-small-map','status':'PASS'})
