...
final class ExpertSettlementPlanner {
    ...
    static PlanningIR plan(HeightfieldMap map,PlanRequest r) {
        ...
        ir.metadata.timestamp=Instant.now().toString();ir.metadata.version=PlanningIR.EXPERT_SCHEMA_VERSION;ir.metadata.randomSeed=r.seed;
        ...
        long siteSeed=r.expert.effectiveSiteSeed(r.seed);
        ...
        for(int a=0;a<count&&st.candidateChecks<st.candidateLimit;a++) {
            ...
            for(int i=0;i<demands.size();i++) {
                ...
                int cluster=i;
                double[] anchor=regionalAnchor(map,r,l,demands.size(),a,mix(siteSeed^((long)i<<24)^a));
                double tx=anchor[0],tz=anchor[1];
                Evaluation selected=select(map,r,l,d,pin,tx,tz,quota,st,mix(siteSeed^((long)a<<36)^d.id().hashCode()));
                ...
                if("clearings".equals(r.expert.sitePreference))decision.rules.add("FLAT_CLEARING_SHORE_AND_ENTRANCE_COST");
        ...
        for(int index=0;index<=choices.size();index++) {
            ...
            if(failure!=null&&recovery&&r.expert.siteSeed==null){
    ...
    private static List<Demand> demands(PlanRequest r,HeightfieldMap map) {
    ...
    private static Evaluation select(HeightfieldMap m,PlanRequest r,Layout l,Demand d,ExpertSettings.Pin pin,double tx,double tz,int quota,SearchStats st,long salt) {
        int plotSpacing="city".equals(r.expert.settlementMode)?Math.max(0,r.parcelConfig.minPlotSpacing-1):r.parcelConfig.minPlotSpacing;
        Set<Long> occupied=BoundedSettlementPlanner.blocked(l,null,plotSpacing);
        ...
        for(int i=0;i<quota&&st.candidateChecks<st.candidateLimit;i++) {
            ...
            double repulsion=0,spacing=SiteMetrics.spacingReference(r.targetPlots,r.expert);
            ...
            double anchorWeight=switch(r.expert.settlementMode){case "city"->5.0;case "town"->7.5;default->10.0;};
            double volume=shape.cells.size()*(double)shape.sizeY;
            double hierarchy=switch(r.expert.settlementMode){case "city"->-shape.sizeY*.34-volume*.0009;case "town"->-shape.sizeY*.12-volume*.00025;default->shape.sizeY*.025;};
            if("civic".equals(role(d)))hierarchy-=shape.sizeY*("city".equals(r.expert.settlementMode)?.18:.04);
            double score=anchor*anchorWeight+terrain*.12+repulsion*8*r.expert.buildingRepulsion+hierarchy;
            if("clearings".equals(r.expert.sitePreference)){
                double slope=0,forest=0;for(int[] cell:shape.cells){int xx=x+cell[0],zz=z+cell[1];slope+=m.getSlope(xx,zz);var obstacle=m.getObstacle(xx,zz);forest+=obstacle==HeightfieldMap.ObstacleType.TREE_TRUNK?1:obstacle==HeightfieldMap.ObstacleType.VEGETATION?.25:0;}
                score+=terrain*1.25+slope/shape.cells.size()*1.4+forest/shape.cells.size()*5;
                if(wd<1000000)score+=Math.min(1,Math.abs(wd-5)/20.0)*.85;
                // Private access length is known before global routing; prefer an accessible doorway.
                score+=c.access().size()*.025;
            }
    ...
    private static double[] regionalAnchor(HeightfieldMap m,PlanRequest r,Layout l,int n,int alternative,long salt){
        ...
        for(int k=0;k<128;k++){
            ...
            double distance=2,mx=0,mz=0;
            for(Candidate c:l.placed){double cx=(c.x()+c.preset().sizeX*.5-m.getMinX())/m.getWidth(),cz=(c.z()+c.preset().sizeZ*.5-m.getMinZ())/m.getDepth();distance=Math.min(distance,(cx-x)*(cx-x)+(cz-z)*(cz-z));mx+=cx;mz+=cz;}
            double value;
            if(l.placed.isEmpty())value=n>=4?-Math.hypot(x-.5,z-.5):Math.hypot(x-.5,z-.5);
            else {mx/=l.placed.size();mz/=l.placed.size();double centroid=Math.hypot(x-mx,z-mz);
                double spread=r.expert.buildingRepulsion*switch(r.expert.settlementMode){case "city"->.08;case "town"->.45;default->1.0;};
                double continuity=switch(r.expert.settlementMode){case "city"->1.35;case "town"->.35;default->0.0;};
                value=distance*spread-centroid*continuity;
            }
            if("clearings".equals(r.expert.sitePreference)){
                double forest=0,slope=0;int samples=0;
                for(int dz=-4;dz<=4;dz+=2)for(int dx=-4;dx<=4;dx+=2)if(m.inBounds(wx+dx,wz+dz)){samples++;slope+=m.getSlope(wx+dx,wz+dz);var ob=m.getObstacle(wx+dx,wz+dz);forest+=ob==HeightfieldMap.ObstacleType.TREE_TRUNK?1:ob==HeightfieldMap.ObstacleType.VEGETATION?.25:0;}
                value-=forest/Math.max(1,samples)*.22+slope/Math.max(1,samples)*.12;
                int wd=l.waterDistances[(wz-m.getMinZ())*m.getWidth()+wx-m.getMinX()];if(wd<1000000)value-=Math.min(1,Math.abs(wd-5)/20.0)*.05;
            }
    ...
    private static PlanningIR reject(PlanningIR ir,String reason,long started) {
        ...
        ir.plots.clear();ir.groundColumns.clear();ir.landUses.clear();ir.transportNetwork=new TransportNetwork();ir.earthworks=new EarthworkReport();ir.search.constructionEdits=0;
