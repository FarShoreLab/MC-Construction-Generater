...
final class BoundedSettlementPlanner {
    ...
    static PlanningIR plan(HeightfieldMap map,PlanRequest req){
        ...
        List<Demand> demands=demands(req,map);List<int[]> gates=gates(map,req);
    ...
    static String validate(HeightfieldMap m, PlanRequest r) {
        ...
        if("single".equals(r.presetPalette)){var p=BuildingPresetRegistry.getInstance().getPreset(r.singlePresetId);if(p!=null&&p.archived)return "PRESET_ARCHIVED";}
        for(var q:r.requirements)if(q!=null&&q.presetId!=null){var p=BuildingPresetRegistry.getInstance().getPreset(q.presetId);if(p!=null&&p.archived)return "PRESET_ARCHIVED";}
        if (r.roadDirections!=8 && r.roadDirections!=12 && r.roadDirections!=16) return "ROAD_DIRECTIONS_MUST_BE_8_12_OR_16";
        if (r.targetPlots<0 || r.targetPlots>64 || r.roadWidth<1 || r.roadWidth>5) return "COUNT_OR_ROAD_WIDTH_OUT_OF_RANGE";
        ...
        for (int i=0;i<r.requirements.size();i++) {
            ...
            if (q==null || q.purpose==null || q.purpose.isBlank() || q.purpose.length()>64 || q.count<1 || q.count>64 ||
                    q.minWidth<3 || q.maxWidth>96 || q.minWidth>q.maxWidth ||
                    q.minDepth<3 || q.maxDepth>96 || q.minDepth>q.maxDepth || q.heightLimit<3 || q.heightLimit>48 ||
        ...
        if (total>64 || r.requirements.size()>64) return "TOTAL_DEMAND_EXCEEDS_64";
    ...
    static List<Demand> demands(PlanRequest req,HeightfieldMap map) {
        ...
        if(requirements.isEmpty()&&!"classic".equals(req.presetPalette))
            requirements.addAll("custom".equals(req.presetPalette)?PresetPalette.selectedRequirements(req):"estates".equals(req.presetPalette)?PresetPalette.capacityRequirements(map,req):PresetPalette.requirements(req.presetPalette,req.singlePresetId,req.targetPlots,req.expert==null?null:req.expert.settlementMode));
        ...
        for (int i=0;i<requirements.size();i++) {
            ...
            for (BuildingPreset p : presets) {
                ...
                for(int entranceIndex=0;entranceIndex<p.entranceCount();entranceIndex++)for (String facing : List.of("NORTH","EAST","SOUTH","WEST")) {
                    BuildingPreset v=p.selectEntrance(entranceIndex).rotateToFacing(facing);
                    for(boolean diagonal : req.diagonalBuildings&&p.sizeX<=32&&p.sizeZ<=32?new boolean[]{false,true}:new boolean[]{false}) {
                        try {
