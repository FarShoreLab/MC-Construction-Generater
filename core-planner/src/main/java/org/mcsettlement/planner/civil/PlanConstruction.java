...
public final class PlanConstruction {
    ...
    public static String pavementBlock(GroundColumn c){
        if(c.surfaceMaterial!=null&&!Set.of("dirt_path","gravel","cobblestone","stone_bricks","coarse_dirt").contains(c.surfaceMaterial))throw new IllegalArgumentException("INVALID_SURFACE_MATERIAL");
        if("coarse_dirt".equals(c.surfaceMaterial)&&!"farmland".equals(c.kind))throw new IllegalArgumentException("INVALID_FIELD_BORDER");
        if("farmland".equals(c.kind))return "coarse_dirt".equals(c.surfaceMaterial)?"minecraft:coarse_dirt":"minecraft:farmland[moisture=0]";
        if("farm_hut".equals(c.kind))return "minecraft:stone_bricks";
        if("pasture".equals(c.kind))return "minecraft:grass_block";
        if(RoadTerrain.raised(c))return "minecraft:spruce_planks";
        String material=("road".equals(c.kind)||"access".equals(c.kind))&&c.surfaceMaterial!=null?c.surfaceMaterial:"cobblestone";
        if(!stair(c))return "minecraft:"+material;
        if(c.facing==null||!Set.of("NORTH","SOUTH","WEST","EAST").contains(c.facing))throw new IllegalArgumentException("INVALID_STAIR_FACING");
        return "minecraft:"+("stone_bricks".equals(material)?"stone_brick_stairs":"cobblestone_stairs")+"[facing="+c.facing.toLowerCase(Locale.ROOT)+",half=bottom,shape=straight,waterlogged=false]";
    }
    public static int columnEditCount(GroundColumn c) {
        return RoadTerrain.raised(c)?c.clearToY-c.targetY+1+(c.support?c.targetY-c.originalY-1:0):c.clearToY-Math.min(c.targetY,c.originalY+1)+1;
    }
    ...
    public static List<Edit> prepare(PlanningIR ir,String theme){
        if(ir==null||ir.metadata==null||!Set.of(PlanningIR.SCHEMA_VERSION,PlanningIR.EXPERT_SCHEMA_VERSION).contains(ir.metadata.version))throw new IllegalArgumentException("UNSUPPORTED_IR_VERSION");
        ...
        if(ir.transportNetwork.directionCount!=8&&ir.transportNetwork.directionCount!=12&&ir.transportNetwork.directionCount!=16)throw new IllegalArgumentException("INVALID_DIRECTION_COUNT");
        if(ir.plots.size()>64||ir.groundColumns.isEmpty()||ir.groundColumns.size()>100000)throw new IllegalArgumentException("INVALID_MANIFEST_SIZE");
        ...
        for(GroundColumn c:ir.groundColumns){
            if(!RoadTerrain.raised(c)||c.support)for(int y=c.originalY+1;y<c.targetY;y++)put(edits,c.x,y,c.z,RoadTerrain.raised(c)?"minecraft:oak_log":"minecraft:cobblestone");
            put(edits,c.x,c.targetY,c.z,pavementBlock(c));
            for(int y=c.targetY+1;y<=c.clearToY;y++)put(edits,c.x,y,c.z,"minecraft:air");
            for(int y=0;y<c.aboveBlocks.size();y++)put(edits,c.x,c.targetY+1+y,c.z,c.aboveBlocks.get(y));
        }
