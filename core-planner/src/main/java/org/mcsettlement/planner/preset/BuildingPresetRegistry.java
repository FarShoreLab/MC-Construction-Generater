...
public class BuildingPresetRegistry {
    ...
    public static final String[] CLASSIC_PRESET_IDS = {
            "artisan_blacksmith",
            "nordic_cottage",
            "highland_watchtower",
            "country_windmill",
    ...
    public static final String[] EXTRA_PRESET_IDS = {
            "square_cabin", "long_cottage", "l_cottage", "t_lodge", "u_inn", "courtyard_house", "cross_hall", "octagonal_lodge", "stepped_workshop", "l_warehouse", "t_cabin", "courtyard_market", "meadow_hut", "timber_cottage", "village_bakery", "canal_rowhouse", "orchard_farmhouse", "artisan_court", "courtyard_hostel", "cloister_market", "stone_bell_tower", "garden_manor", "merchant_guild", "harvest_barn", "estate_48", "estate_64", "estate_96"
    };
    public static final String[] BUILTIN_PRESET_IDS = java.util.stream.Stream.concat(
            Arrays.stream(CLASSIC_PRESET_IDS),Arrays.stream(EXTRA_PRESET_IDS)).toArray(String[]::new);
    public static final Set<String> CLASSIC_IDS = Set.of(CLASSIC_PRESET_IDS);
    ...
    public List<BuildingPreset> getAllPresets() {
        return presets.values().stream().filter(p->!p.archived).collect(Collectors.toList());
    }
    ...
    public BuildingPreset resolveBestPreset(String prompt, String tagOrCategory, String theme, long seed) {
        ...
        if (tagOrCategory != null && !tagOrCategory.isBlank()) {
            ...
            for (BuildingPreset p : getAllPresets()) {
