...
public final class PresetPalette {
    public static final Set<String> NAMES=Set.of("classic","mixed","compact","spacious","single","crafted","estates","custom");
    ...
    public static List<BuildingRequirement> selectedRequirements(org.mcsettlement.planner.SettlementPlanner.PlanRequest req){
        if(req.expert==null)throw new IllegalArgumentException("CUSTOM_LIST_REQUIRES_EXPERT");
        var selections=org.mcsettlement.planner.ExpertSettings.parseBuildingSelection(new com.google.gson.Gson().toJson(req.expert.buildingSelection));
        List<BuildingRequirement> out=new ArrayList<>();int total=0;
        for(var item:selections)if(item.enabled){var q=requirements("single",item.presetId,1).getFirst();q.id="selected_"+item.presetId;q.count=item.count;q.placement=out.isEmpty()?"center":"any";out.add(q);total+=item.count;}
        if(total<1||total>64||total!=req.targetPlots)throw new IllegalArgumentException("BUILDING_COUNTS_MUST_MATCH_TARGET_1_TO_64");
        if(req.expert.pins.stream().anyMatch(p->"building".equals(p.kind)))throw new IllegalArgumentException("CUSTOM_LIST_CANNOT_REPLACE_MANUAL_PINS");
        return out;
    }
    ...
    public static List<BuildingRequirement> capacityRequirements(org.mcsettlement.planner.terrain.HeightfieldMap map,
        ...
        int maxEstates=0,shortSide=Math.min(map.getWidth(),map.getDepth());
