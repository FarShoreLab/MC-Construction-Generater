...
public class SimulatedSettlementPipeline {
    ...
    public static PipelineResult run(int width,int depth,int baseElevation,int relief,String terrainType,long terrainSeed,long planSeed,int targetPlots,
        ...
        return run(width,depth,baseElevation,relief,terrainType,terrainSeed,planSeed,targetPlots,roadDirections,diagonalBuildings,budget,presetPalette,singlePresetId,terrainParameters,expertSettings,"cobblestone");
    }
    public static PipelineResult run(int width,int depth,int baseElevation,int relief,String terrainType,long terrainSeed,long planSeed,int targetPlots,
            int roadDirections,boolean diagonalBuildings,SettlementPlanner.SearchBudget budget,String presetPalette,String singlePresetId,
            TerrainParameters terrainParameters,org.mcsettlement.planner.ExpertSettings expertSettings,String roadSurface) {
        if(targetPlots<1||targetPlots>64||(roadDirections!=8&&roadDirections!=12&&roadDirections!=16))throw new IllegalArgumentException("INVALID_PLANNING_PARAMETERS");
        ...
        res.expertSettings=expertSettings;res.roadSurface=roadSurface;
        ...
        if ("plateau".equals(res.terrainType)) {
            // Five-block terrace risers need up to four blocks of approach excavation.
            req.roadMaxCut=8;req.roadMaxFill=8;
        }
        PlanningIR ir = SettlementPlanner.plan(map, req);
        res.plan = ir;
        res.planSeed=ir.metadata.randomSeed;
    ...
    public static String hashPlan(PlanningIR ir) {
        try {
            ...
            for (Plot plot : ir.plots) {
                ...
                if(plot.builder.entranceIndex!=0)updateInt(digest,plot.builder.entranceIndex);
            ...
            ir.groundColumns.stream()
                    ...
                    .forEach(c -> {
                        ...
                        if(c.surfaceMaterial!=null&&!"cobblestone".equals(c.surfaceMaterial))updateString(digest,c.surfaceMaterial);
                        for(String block:c.aboveBlocks)updateString(digest,block);
    ...
    private static VoxelType simulationMaterial(String id) {
        ...
        if(id.startsWith("minecraft:stone_brick_stairs"))return VoxelType.STONE_BRICK_STAIRS;
        if(id.equals("minecraft:white_terracotta"))return VoxelType.PLASTER;
        if(id.equals("minecraft:bricks"))return VoxelType.BRICKS;
        if(id.equals("minecraft:deepslate_tiles"))return VoxelType.SLATE;
        if(id.equals("minecraft:dark_oak_planks"))return VoxelType.DARK_OAK;
        if(id.equals("minecraft:oak_planks"))return VoxelType.OAK_PLANKS;
        ...
        if (id.startsWith("minecraft:farmland")) return VoxelType.FARMLAND;
        if (id.equals("minecraft:grass_block")) return VoxelType.GRASS_BLOCK;
