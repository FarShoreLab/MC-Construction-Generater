...
import org.mcsettlement.planner.ExpertSettings;
...
public final class SimulationApiRunner {
    ...
    public static void main(String[] args)throws IOException{
        ...
        try{System.setOut(new PrintStream(System.err,true));
            ...
            Object payload;
            if (request.terrainOnly) {
                PipelineResult result=SimulatedSettlementPipeline.generateTerrain(request.width,request.depth,request.baseElevation,
                        request.relief,request.terrainType,request.terrainSeed,request.terrainParameters);
                payload=new TerrainPayload(result);
            } else {
                PipelineResult result=SimulatedSettlementPipeline.run(request.width,request.depth,request.baseElevation,request.relief,
                        request.terrainType,request.terrainSeed,request.planSeed,request.targetPlots,request.roadDirections,
                        request.diagonalBuildings,request.budget,request.presetPalette,request.singlePresetId,request.terrainParameters,request.expertSettings,request.roadSurface);
                payload=new Payload(result);
            }
            byte[] data=encode(payload);out.write(data);
    ...
    private static final class Request{
        ...
        boolean terrainOnly;
        String roadSurface="cobblestone";String layoutMode="expert";ExpertSettings expertSettings=new ExpertSettings();
        TerrainParameters terrainParameters;
        Map<String,String> terrainOverrides=new LinkedHashMap<>();
        static Request parse(String[] args){Request r=new Request();
            for(String arg:args){if(!arg.startsWith("--")||!arg.contains("="))throw new IllegalArgumentException("INVALID_ARGUMENT");String[] pair=arg.substring(2).split("=",2);String v=pair[1];
                switch(pair[0]){case "width"->r.width=Integer.parseInt(v);case "depth"->r.depth=Integer.parseInt(v);case "baseElevation"->r.baseElevation=Integer.parseInt(v);case "relief"->r.relief=Integer.parseInt(v);
                    ...
                    case "roadMergeDistance"->r.expertSettings.roadMergeDistance=Integer.parseInt(v);
                    case "maxBridgeSpan"->r.expertSettings.maxBridgeSpan=Integer.parseInt(v);
                    case "maxLandBridgeSpan"->r.expertSettings.maxLandBridgeSpan=Integer.parseInt(v);
                    case "buildingSelection"->r.expertSettings.buildingSelection=ExpertSettings.parseBuildingSelection(v);
                    case "sitePreference"->r.expertSettings.sitePreference=v;
                    case "settlementMode"->r.expertSettings.settlementMode=v;
                    case "maxPlanAttempts"->r.expertSettings.maxPlanAttempts=Integer.parseInt(v);
                    case "siteSeed"->r.expertSettings.siteSeed="follow".equals(v)?null:Long.parseLong(v);
                    case "pins"->r.expertSettings.pins=ExpertSettings.parsePins(v);
                    case "allowBridges","autoDock"->{if(!Set.of("true","false").contains(v))throw new IllegalArgumentException("INVALID_BOOLEAN");if("allowBridges".equals(pair[0]))r.expertSettings.allowBridges=Boolean.parseBoolean(v);else r.expertSettings.autoDock=Boolean.parseBoolean(v);}
                    ...
                    case "terrainOnly"->{if(!Set.of("true","false").contains(v))throw new IllegalArgumentException("INVALID_BOOLEAN");r.terrainOnly=Boolean.parseBoolean(v);}
            ...
            r.terrainParameters=TerrainParameters.fromOverrides(r.terrainType,r.terrainOverrides);
            if(!Set.of("expert","legacy").contains(r.layoutMode))throw new IllegalArgumentException("INVALID_LAYOUT_MODE");
            if(!Set.of("dirt_path","gravel","cobblestone","stone_bricks").contains(r.roadSurface))throw new IllegalArgumentException("INVALID_ROAD_SURFACE");
            if("single".equals(r.presetPalette)&&BuildingPresetRegistry.getInstance().getPreset(r.singlePresetId).archived)throw new IllegalArgumentException("PRESET_ARCHIVED");
            r.expertSettings.validate(r.width,r.depth,0,0,r.targetPlots);
            if("legacy".equals(r.layoutMode)){if(!r.expertSettings.pins.isEmpty())throw new IllegalArgumentException("LEGACY_MODE_DOES_NOT_ACCEPT_PINS");r.expertSettings=null;}
            return r;
        }
    }
    ...
    private record Config(int width,int depth,int baseElevation,int relief,String terrainType,long terrainSeed,long planSeed,int targetPlots,int roadDirections,boolean diagonalBuildings,String presetPalette,String singlePresetId,String originalTerrainHash,
        ...
        Config(PipelineResult r){this(r.width,r.depth,r.baseElevation,r.relief,r.terrainType,r.terrainSeed,r.planSeed,r.targetPlots,r.roadDirections,r.diagonalBuildings,r.presetPalette,r.singlePresetId,r.originalTerrainHash,
                r.terrainParameters,TerrainBlockGenerator.GENERATOR_VERSION,
                TerrainBlockGenerator.waterLevel(r.baseElevation,r.relief,r.terrainParameters),r.expertSettings==null?"legacy":"expert",r.expertSettings,r.roadSurface);}}
    /** No plan/constructed layer is fabricated for terrain-only requests. */
    private static final class TerrainPayload {
        final boolean ok=true;
        final String kind="TERRAIN_ONLY", schemaVersion="terrain-preview/1";
        final TerrainConfig config;
        final TerrainMetrics metrics;
        final Layer original;
        TerrainPayload(PipelineResult r){config=new TerrainConfig(r);original=new Layer(r,false);metrics=new TerrainMetrics(r,original);}
    }
    private record TerrainConfig(int width,int depth,int baseElevation,int relief,String terrainType,long terrainSeed,
            String originalTerrainHash,TerrainParameters terrainParameters,String terrainGeneratorVersion,int waterLevel){
        TerrainConfig(PipelineResult r){this(r.width,r.depth,r.baseElevation,r.relief,r.terrainType,r.terrainSeed,
                r.originalTerrainHash,r.terrainParameters,TerrainBlockGenerator.GENERATOR_VERSION,
                TerrainBlockGenerator.waterLevel(r.baseElevation,r.relief,r.terrainParameters));}
    }
    private static final class TerrainMetrics {
        final String status="TERRAIN_ONLY";
        final int minSurface,maxSurface,waterCells,treeCount;
        final double meanSurface,waterFraction,meanNeighborStep;
        final int responseByteLimit=MAX_RESPONSE_BYTES,extraVoxelLimit=MAX_EXTRAS_PER_LAYER;
        TerrainMetrics(PipelineResult r,Layer layer){
            int min=Integer.MAX_VALUE,max=Integer.MIN_VALUE,water=0,trees=0;long sum=0,steps=0,edges=0;
            for(int z=0;z<r.depth;z++)for(int x=0;x<r.width;x++){
                int i=z*r.width+x,h=layer.heights[i];min=Math.min(min,h);max=Math.max(max,h);sum+=h;
                if(layer.waters[i]>=0)water++;
                if(r.worldBefore.getBlock(x,h+1,z)==SimulatedVoxelWorld.VoxelType.OAK_LOG)trees++;
                if(x+1<r.width){steps+=Math.abs(h-layer.heights[i+1]);edges++;}
                if(z+1<r.depth){steps+=Math.abs(h-layer.heights[i+r.width]);edges++;}
            }
            minSurface=min;maxSurface=max;waterCells=water;treeCount=trees;
            meanSurface=sum/(double)layer.heights.length;waterFraction=water/(double)layer.heights.length;
            meanNeighborStep=edges==0?0:steps/(double)edges;
        }
    }
    private static final class Metrics{
        ...
        final SitePlanning sitePlanning;final SearchStats search;final NetworkMetrics network;final String networkAlgorithm;final int responseByteLimit=MAX_RESPONSE_BYTES,extraVoxelLimit=MAX_EXTRAS_PER_LAYER;
    ...
    private static final class Layer{
        ...
        Layer(PipelineResult r,boolean after){SimulatedVoxelWorld world=after?r.worldAfter:r.worldBefore;minY=world.getMinY();sizeX=world.getSizeX();sizeY=world.getSizeY();sizeZ=world.getSizeZ();
            ...
            for(int z=0;z<sizeZ;z++)for(int x=0;x<sizeX;x++){int index=z*sizeX+x;GroundColumn c=columns.get(key(x,z));
                int surface=c==null?r.heightfield.getSurfaceY(x,z):org.mcsettlement.planner.RoadTerrain.raised(c)?c.originalY:c.targetY-("stair".equals(c.structure)?1:0);
    ...
    private record PresetInfo(String id,String name,String shape,String sizeTier,int width,int depth,int height,int area,List<String> mask,int entranceCount,String description) {}
    private static List<PresetInfo> catalog(){List<PresetInfo> out=new ArrayList<>();for(var p:BuildingPresetRegistry.getInstance().getAllPresets()){
        ...
        out.add(new PresetInfo(p.id,p.name,p.footprintShape,p.sizeTier,p.sizeX,p.sizeZ,p.sizeY,p.footprintArea(),mask,p.entranceCount(),p.description));}return out;}
    ...
    private static final class PlanPreview{
        final SitePlanning sitePlanning;final String status,hash;final List<PlotPreview> plots=new ArrayList<>();final List<GroundColumn> groundColumns;final List<LandUseArea> landUses;
