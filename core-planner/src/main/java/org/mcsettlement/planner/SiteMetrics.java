...
public final class SiteMetrics {
    ...
    public static double spacingReference(int count,ExpertSettings settings){return spacingReference(count)*settings.spacingScale();}
    public static void measure(HeightfieldMap m,List<Plot> plots,ExpertSettings settings,SitePlanning out) {
        out.minBBoxCoverage=settings.minBBoxCoverage*settings.coverageScale();out.minMinorAxisRatio=settings.minMinorAxisRatio*settings.axisScale();
        out.minHullCoverage=plots.size()>=3?out.minBBoxCoverage*.35:0;
        out.spacingReference=spacingReference(plots.size(),settings);out.closePairs=out.crowdedBuildings=0;
        ...
        out.coveragePenalty=square(Math.max(0,out.minBBoxCoverage-out.bboxCoverage))+square(Math.max(0,out.minHullCoverage-out.hullCoverage));
        if(plots.size()>=3)out.coveragePenalty+=square(Math.max(0,out.minMinorAxisRatio-out.minorAxisRatio));
    }
