...
def main() -> None:
    ...
    def fetch(params:dict) -> tuple[int,bytes,float]:
        begin=time.perf_counter();url=args.base+'/api/plan?'+urllib.parse.urlencode(params)
    ...
    def run(name:str,params:dict) -> dict:
        ...
        if name in {'size512','reference512','loop128','mixed128'}:
    ...
    for case in [{'width':513},{'depth':0},{'roadDirections':10},{'diagonalBuildings':'yes'},{'planSeed':9007199254740992},{'pathStates':200001},{'constructionEdits':1000001},{'width':'128.1'},{'unknownParameter':1},{'presetPalette':'silent_fallback'},{'singlePresetId':'unknown_preset'},{'singlePresetId':'../town_hall'},{'presetPalette':'single','singlePresetId':'/etc/passwd'}]:
        code,raw,_=fetch(case);assert code==400 and not json.loads(raw)['ok'],case
    checks.append('Thirteen invalid bounds/types/options rejected by HTTP 400')
