...
public class PlannerTest {
    ...
    public void testEndToEndSettlementPlanningAndIRSerialization() {
        HeightfieldMap map = new HeightfieldMap(0, 0, 80, 80);
        for (int x=0;x<80;x++) for(int z=0;z<80;z++) map.setSurfaceY(x,z,64);
        ...
        assertEquals("COMPLETE", ir.status);
        assertEquals(4, ir.plots.size());
        assertTrue(ir.unmetRequirements.isEmpty());
        assertFalse(org.mcsettlement.planner.civil.PlanConstruction.prepare(ir, req.settlementStyle).isEmpty());

        // Test serialization round-trip
        String json = ir.toJson(true);
        assertNotNull(json);
        assertTrue(json.contains("locked_preset"));
