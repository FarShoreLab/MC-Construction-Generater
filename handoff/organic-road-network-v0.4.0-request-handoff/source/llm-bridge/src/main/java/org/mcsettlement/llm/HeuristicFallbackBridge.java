...
public class HeuristicFallbackBridge {
    ...
    public static PlanningIntent generateIntent(HeightfieldMap map, String userPrompt) {
        ...
        // Keywords request ONE local-purpose building, not the same building on every plot.
        String[][] keywords = {
            {"workshop","artisan_blacksmith","铁匠|锻造|blacksmith|forge"},
            {"commercial","adventurer_tavern","酒馆|旅馆|tavern|inn"},
            {"military","highland_watchtower","哨塔|watchtower"},
            {"agriculture","country_windmill","风车|windmill"},
            {"agriculture","farmstead_granary","谷仓|粮仓|granary|barn"},
            {"government","town_hall","市政|政厅|town hall"},
            {"culture","village_chapel","教堂|chapel|church"},
            {"mystic","wizard_tower","法师|wizard"},
            {"industry","lumber_mill","伐木|lumber"}
        };
        for (String[] group : keywords) {
            if (java.util.Arrays.stream(group[2].split("\\|")).anyMatch(promptLower::contains)) {
                var requirement = new org.mcsettlement.planner.SettlementPlanner.BuildingRequirement();
                requirement.id = group[1]; requirement.purpose = group[0]; requirement.presetId = group[1];
                if ("government".equals(group[0]) || "military".equals(group[0])) requirement.placement = intent.landmarkPlacement;
                intent.requirements.add(requirement);
            }
        }
        if (!intent.requirements.isEmpty()) {
            int housing = Math.max(1, intent.targetPlotCount - intent.requirements.size());
            var requirement = new org.mcsettlement.planner.SettlementPlanner.BuildingRequirement();
            requirement.id = "housing"; requirement.count = housing;
            intent.requirements.add(requirement);
            intent.targetPlotCount = intent.requirements.stream().mapToInt(q -> q.count).sum();
        }
        return intent;
    }
}
