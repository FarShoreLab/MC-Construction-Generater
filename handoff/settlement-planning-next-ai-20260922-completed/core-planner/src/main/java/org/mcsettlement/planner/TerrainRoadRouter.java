...
final class TerrainRoadRouter {
    ...
    static final class Trace { String failure="NO_ROUTE_WITHIN_CONSTRAINTS"; int rejectedGoals; boolean crossLink, skipReuse; }
    private record Node(long id,int cell,int y,int heading,int rise,int run,int wetRun,int landRun,int g,int f,Node parent) {}
    private static long id(int cell,int y,int heading,int rise,int run,int wetRun,int landRun){return ((((((long)cell*4096+y+2032)*17+heading)*3+rise+1)*25+run)*97+wetRun)*33+landRun;}
    ...
    static Result routeTo(HeightfieldMap map,PlanRequest req,Set<Long> forbidden,Set<Long> roads,
        ...
        if(req.expert!=null&&req.expert.roadMergeDistance>0&&!trace.crossLink&&!trace.skipReuse&&goals.size()<=4){
            Result reused=reuseKnownCenterline(map,req,forbidden,roads,fixed,access,accesses,previousRoutes,entry,stats,end,goals);
            if(reused!=null)return reused;
        }
        int cell=(start.z-map.getMinZ())*w+start.x-map.getMinX(),heading=dirs.length;
        if(distances[cell]>=(req.expert==null?960:320))distances=coarseBarrierHeuristic(map,req,forbidden,goals,distances,stats,end);
        RoadProximity proximity=req.expert!=null&&(req.expert.roadMergeDistance>0||trace.crossLink)?
            new RoadProximity(map,previousRoutes,req.expert.roadMergeDistance,stats):null;
        int endpointRadius=req.roadWidth*2+3;
        // Weight 3 biases the mixed movement set toward repeating long knight steps, even
        // on a flat horizontal road. Weight 2 avoids that local tie bias while retaining
        // bounded weighted A*; legacy 8/12 and their replay hashes remain unchanged.
        // Chord search deliberately leaves the served tree. Preserve the more focused
        // weight for this optional phase; full pavement and macro-loop checks remain hard.
        int heuristicWeight=req.expert!=null&&req.roadDirections==16&&!trace.crossLink?2:3;
        // On terraced terrain, surface-height penalties otherwise exhaust local quotas at
        // each riser. Focus the bounded search; the pavement solver still certifies every move.
        if(req.expert!=null&&req.expert.allowBridges&&map.hasDerivedCliffs())heuristicWeight=8;
        int[] endpointDistances=proximity==null?null:distanceField(map,goals);
        ...
        long rootId=id(cell,start.y,startHeading,startRise,0,startWet,0);
        Node root=new Node(rootId,cell,start.y,startHeading,startRise,0,startWet,0,0,heuristicWeight*(guide==null?distances[cell]:guide.heuristic(cell,distances[cell])),null);best.put(rootId,root);open.add(root);stats.peakPathStates=Math.max(stats.peakPathStates,1);stats.peakOpenNodes=Math.max(stats.peakOpenNodes,1);
        int rejected=0;Set<Long> rejectedGoalCells=new HashSet<>();
        while(!open.isEmpty()&&stats.pathExpanded<end&&stats.pathExpanded<stats.pathLimit){
            ...
            if(goals.contains(k)&&goal!=null&&goal.targetY==n.y&&(req.expert==null||!rejectedGoalCells.contains(k))){
                ...
                if(req.expert!=null)rejectedGoalCells.add(k);
                trace.rejectedGoals=++rejected;
                if(rejected>=(req.expert==null?6:12)||stats.gradeRelaxations>=stats.gradeLimit){trace.failure=stats.gradeRelaxations>=stats.gradeLimit?"GRADE_RELAXATIONS":"GOAL_GRADE_OR_STAIRS_REJECTED";return null;}
            }
            for(int di=0;di<dirs.length;di++){
                ...
                for(int rise:new int[]{0,-1,1}){
                    ...
                    if(req.expert!=null&&req.expert.allowBridges&&rise!=0){
                        int hx=rise>0?nx:x,hz=rise>0?nz:z,hy=Math.max(n.y,y);
                        if(!RoadTerrain.wet(map,hx,hz)&&hy-map.getSurfaceY(hx,hz)>req.roadMaxFill)continue;
                    }
                    ...
                    int movement=dx==0||dz==0?10:Math.abs(dx)+Math.abs(dz)==2?14:22;
                    boolean endpoint=proximity!=null&&(Math.hypot(nx-start.x,nz-start.z)<=endpointRadius||endpointDistances[next]<=endpointRadius*10);
                    int nearby=proximity==null?0:proximity.cost(next,y,dx,dz,endpoint,trace.crossLink);
                    // Two-step bearing prevents alternating diagonals from evading a parallel-lane
                    // cost. Crossings remain exempt; this is a soft cost, not geometry snapping.
                    if(proximity!=null&&nearby==0&&n.parent!=null){int bx=nx-(n.parent.cell%w+map.getMinX()),bz=nz-(n.parent.cell/w+map.getMinZ());
                        if(bx!=0||bz!=0)nearby=proximity.cost(next,y,bx,bz,endpoint,trace.crossLink);}
                    // Shared geometry stays in the one existing manifest. Grade/full-width validation
                    // still decides whether this move can actually reuse that centerline.
                    if(proximity!=null&&!trace.crossLink&&proximity.center(n.cell)&&proximity.center(next))movement=Math.max(5,movement*3/5);
                    int roadRoll=req.expert==null?0:roadRollBias(req.seed,nx,nz,di);
                    int g=n.g+movement+nearby+soil+roadRoll+Math.abs(rise)*7+turn+(n.rise*rise<0?12:0)+(guide==null?0:guide.cost(next)+guide.straightCost(n.run,di==n.heading));
                    int run=guide==null?0:di==n.heading?Math.min(24,n.run+1):1;
                    long nextId=id(next,y,di,rise,run,wetRun,landRun);Node old=best.get(nextId);if(old!=null&&g>=old.g)continue;
                    ...
                    Node value=new Node(nextId,next,y,di,rise,run,wetRun,landRun,g,g+heuristicWeight*(guide==null?distances[next]:guide.heuristic(next,distances[next])),n);best.put(nextId,value);open.add(value);
        ...
        else if(stats.pathExpanded>=end){if(req.expert==null)exhausted(stats,"ROUTE_OR_SLOT_EXPANSIONS");else stats.routeQuotaExhaustions++;trace.failure="ROUTE_OR_SLOT_EXPANSIONS";}
    ...
    private record SharedArc(long next,int cost) {}
    private record SharedState(long cell,int cost) {}
    /** A required endpoint connection need not excavate another parallel road. Join a nearby
     * committed centerline, then follow its certified graph. The join AND concatenated route
     * are revalidated against the same whole-pavement solver. At most 2048 shared-graph pops
     * and 1500 local-join expansions, all charged to the ORIGINAL route/global path budget. */
    private static Result reuseKnownCenterline(HeightfieldMap map,PlanRequest req,Set<Long> forbidden,Set<Long> roads,
            Map<Long,GroundColumn> fixed,List<RoadStep> access,List<List<RoadStep>> accesses,
            List<List<RoadStep>> previous,int[] entry,SearchStats stats,int end,Set<Long> goals){
        if(previous.isEmpty()||stats.pathExpanded>=end)return null;RoadStep start=access.getLast();long source=key(start.x,start.z);
        Map<Long,List<SharedArc>> graph=new TreeMap<>();
        for(var route:previous)for(int i=1;i<route.size();i++){
            RoadStep a=route.get(i-1),b=route.get(i);long u=key(a.x,a.z),v=key(b.x,b.z);stats.roadReuseGraphVisits++;
            if(!fixed.containsKey(u)||!fixed.containsKey(v))continue;int cost=(int)Math.round(Math.hypot(a.x-b.x,a.z-b.z)*10)+Math.abs(a.y-b.y)*7;
            graph.computeIfAbsent(u,k->new ArrayList<>()).add(new SharedArc(v,cost));graph.computeIfAbsent(v,k->new ArrayList<>()).add(new SharedArc(u,cost));
        }
        Set<Long> joins=new TreeSet<>();int radius=req.expert.roadMergeDistance;
        for(long k:graph.keySet())if(Math.abs(x(k)-start.x)+Math.abs(z(k)-start.z)<=radius&&Math.abs(fixed.get(k).targetY-start.y)<=1)joins.add(k);
        if(joins.isEmpty()||joins.stream().allMatch(goals::contains))return null;
        Map<Long,Integer> distance=new HashMap<>();Map<Long,Long> toward=new HashMap<>();
        PriorityQueue<SharedState> queue=new PriorityQueue<>(Comparator.comparingInt(SharedState::cost).thenComparingLong(SharedState::cell));
        for(long k:new TreeSet<>(goals))if(graph.containsKey(k)){distance.put(k,0);queue.add(new SharedState(k,0));}
        int limit=Math.min(Math.min(end,stats.pathLimit),stats.pathExpanded+2048);long selected=0;double best=Double.POSITIVE_INFINITY;boolean found=false;
        Set<Long> settled=new HashSet<>();
        while(!queue.isEmpty()&&stats.pathExpanded<limit){SharedState u=queue.remove();if(u.cost!=distance.get(u.cell)||!settled.add(u.cell))continue;stats.pathExpanded++;stats.roadReuseExpanded++;
            if(joins.contains(u.cell)){
                double score=u.cost+Math.hypot(x(u.cell)-start.x,z(u.cell)-start.z)*10;
                if(score<best){best=score;selected=u.cell;found=true;}if(u.cell==source)break;
            }
            for(SharedArc edge:graph.getOrDefault(u.cell,List.of())){int value=u.cost+edge.cost;if(value<distance.getOrDefault(edge.next,Integer.MAX_VALUE)){distance.put(edge.next,value);toward.put(edge.next,u.cell);queue.add(new SharedState(edge.next,value));}}
        }
        if(!found||stats.pathExpanded>=end)return null;
        double direct=goals.stream().mapToDouble(k->Math.hypot(x(k)-start.x,z(k)-start.z)*10).min().orElse(0);
        if(selected!=source&&(distance.get(selected)<120||best>Math.max(120,direct*2.25)))return null;
        List<RoadStep> path;
        if(selected==source)path=new ArrayList<>(List.of(start));
        else{
            Trace joining=new Trace();joining.skipReuse=true;Set<Long> landing=Set.of(selected);
            Result local=routeTo(map,req,forbidden,roads,fixed,access,accesses,previous,entry,distanceField(map,landing),stats,Math.min(end,stats.pathExpanded+1500),landing,null,joining);
            if(local==null)return null;path=new ArrayList<>(local.centers);
        }
        long k=selected;Set<Long> visited=new HashSet<>();for(RoadStep p:path)if(!visited.add(key(p.x,p.z)))return null;
        int shared=0;while(!goals.contains(k)){
            Long next=toward.get(k);if(next==null||!visited.add(next))return null;GroundColumn c=fixed.get(next);path.add(step(c));k=next;shared++;
        }
        if(shared==0)return null;
        Map<Long,GroundColumn> anchors=new TreeMap<>(fixed);for(RoadStep p:access)anchors.putIfAbsent(key(p.x,p.z),RoadTerrain.column(map,p.x,p.z,p.y,access.size()==1?"road":p==access.getFirst()?"foundation":"access",p.y+3));
        Set<Long> area=new TreeSet<>(roads);area.addAll(footprint(path,req.roadWidth));
        var solved=PavementGrades.solve(map,req,area,anchors,path,accesses,previous,entry,stats);
        if(solved==null)return null;stats.reusedRouteCount++;stats.reusedCenterlineSteps+=shared;
        return new Result(path,area,solved);
    }
    ...
    private static int[] coarseBarrierHeuristic(HeightfieldMap map,PlanRequest req,Set<Long> forbidden,Set<Long> goals,int[] original,SearchStats stats,int end){
        ...
        boolean[] bridgeEligible=null;
        if(req.expert!=null){
            bridgeEligible=new boolean[w*d];int limit=Math.max(1,req.expert.maxBridgeSpan-req.roadWidth);
            // A soft guide mask: complete straight wet runs short enough to be a possible bridge.
            // Wide water can still be searched by the exact router (e.g. a pinned dock), but cannot
            // masquerade as a cheap overland shortcut in every large-map coarse heuristic.
            for(int axis=0;axis<2;axis++){
                int lines=axis==0?d:w,len=axis==0?w:d;
                for(int line=0;line<lines;line++)for(int k=0;k<len;){
                    int x=axis==0?k:line,z=axis==0?line:k;stats.terrainGuideCellVisits++;
                    if(!RoadTerrain.wet(map,x+map.getMinX(),z+map.getMinZ())){k++;continue;}
                    int a=k;while(k<len){x=axis==0?k:line;z=axis==0?line:k;
                        if(!RoadTerrain.wet(map,x+map.getMinX(),z+map.getMinZ()))break;k++;stats.terrainGuideCellVisits++;}
                    if(a>0&&k<len&&k-a<=limit)for(int j=a;j<k;j++)bridgeEligible[(axis==0?line:j)*w+(axis==0?j:line)]=true;
                }
            }
        }
        for(int z=0;z<cd;z++)for(int x=0;x<cw;x++){
            outer:for(int dz=0;dz<8;dz++)for(int dx=0;dx<8;dx++){
                ...
                stats.terrainGuideCellVisits++;
                boolean usable=req.expert==null||!RoadTerrain.wet(map,wx,wz)||map.inBounds(wx,wz)&&bridgeEligible[(wz-map.getMinZ())*w+wx-map.getMinX()];
                if(usable&&RoadTerrain.allowed(map,req,wx,wz)&&!forbidden.contains(key(wx,wz))){open[z*cw+x]=true;break outer;}
            }
        }
    ...
    /** Small deterministic tie-breaker: rolling planSeed can change roads while siteSeed stays fixed. */
    private static int roadRollBias(long seed,int x,int z,int heading){
        long v=seed^((long)x*0x9e3779b97f4a7c15L)^((long)z*0xc2b2ae3d27d4eb4fL)^((long)heading*0x165667b19e3779f9L);
        v^=v>>>33;v*=0xff51afd7ed558ccdL;v^=v>>>33;return (int)(v&3L);
    }
}
