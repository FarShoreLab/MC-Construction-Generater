...
final class PavementGrades {
    ...
    static Map<Long,GroundColumn> solve(HeightfieldMap map,PlanRequest req,Set<Long> footprint,
        ...
        if(req.expert!=null&&!RoadTerrain.validLandSpans(out,req.expert.maxLandBridgeSpan))return fail(stats,"LAND_BRIDGE_SPAN");
        if(req.expert!=null){for(List<RoadStep> route:previousRoutes)if(!RoadTerrain.validLandDeckRun(out,route,req.roadWidth,req.expert.maxLandBridgeSpan))return fail(stats,"LAND_BRIDGE_CONTINUOUS_RUN");
            if(!RoadTerrain.validLandDeckRun(out,centers,req.roadWidth,req.expert.maxLandBridgeSpan))return fail(stats,"LAND_BRIDGE_CONTINUOUS_RUN");}
        return out;
    }
