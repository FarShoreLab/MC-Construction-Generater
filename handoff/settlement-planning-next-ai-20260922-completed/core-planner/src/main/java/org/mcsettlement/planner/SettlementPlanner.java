...
public final class SettlementPlanner {
    ...
    public static PlanningIR plan(HeightfieldMap map, PlanRequest request) {
        if(request==null||request.expert==null)return BoundedSettlementPlanner.plan(map,request);
        ...
        for(int round=1;round<=limit;round++){
            ...
            if(!"REJECTED".equals(ir.status))return ir;
        }
    ...
    private static PlanRequest copyForRetry(PlanRequest src,long seed){
        ...
        r.parcelConfig=src.parcelConfig;r.requirements=new ArrayList<>(src.requirements);r.searchBudget=src.searchBudget;r.roadMaxCut=src.roadMaxCut;r.roadMaxFill=src.roadMaxFill;
