...
public final class ExpertTerrainAudit {
    ...
    private static void validateLandUses(HeightfieldMap map,PlanningIR ir,Map<Long,PlanningIR.GroundColumn> columns){
        ...
        for(var area:ir.landUses){if(!Set.of("farmland","pasture").contains(area.type)||area.cells.size()<12)throw new IllegalArgumentException("AUDIT_INVALID_LAND_USE");
            ...
            if(cells.size()==(maxX-minX+1)*(maxZ-minZ+1))throw new IllegalArgumentException("AUDIT_RECTANGULAR_LAND_USE");
            for(int[] p:area.boundary2D)if(!cells.contains(key(p[0],p[1])))throw new IllegalArgumentException("AUDIT_LAND_USE_BOUNDARY");
        }
        long actual=ir.groundColumns.stream().filter(c->"farmland".equals(c.kind)||"pasture".equals(c.kind)).count();if(actual!=declared.size())throw new IllegalArgumentException("AUDIT_ORPHAN_LAND_USE_COLUMN");
    }
