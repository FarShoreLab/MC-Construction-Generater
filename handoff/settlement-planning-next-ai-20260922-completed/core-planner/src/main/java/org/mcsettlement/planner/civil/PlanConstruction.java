...
public final class PlanConstruction {
    ...
    public static String pavementBlock(GroundColumn c){
        if("farmland".equals(c.kind))return "minecraft:farmland[moisture=7]";
    ...
    public static List<Edit> prepare(PlanningIR ir,String theme){
        ...
        for(GroundColumn c:ir.groundColumns){
            if(!Set.of("road","access","foundation","farmland","pasture").contains(c.kind)||!Set.of("surface","stair","bridge","deck").contains(c.structure)||
            ...
            if(("farmland".equals(c.kind)||"pasture".equals(c.kind))&&(c.targetY!=c.originalY||!"surface".equals(c.structure)||c.waterY!=null||c.support||c.facing!=null))throw new IllegalArgumentException("INVALID_LAND_USE_COLUMN");
    ...
    private static void validateWalk(PlanningIR ir,Map<Long,GroundColumn> cols){
        ...
        for(GroundColumn c:cols.values())if(!Set.of("foundation","farmland","pasture").contains(c.kind)&&!position.containsKey(key(c.x,c.z)))throw new IllegalArgumentException("PAVEMENT_WITHOUT_NODE");
