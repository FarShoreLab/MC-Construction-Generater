...
public final class NetworkDiversityMain {
    ...
    public static void main(String[] args)throws Exception{
        ...
        test("mixed-state-key-no-sentinel-or-heading-alias",()->{Method method=TerrainRoadRouter.class.getDeclaredMethod("id",int.class,int.class,int.class,int.class,int.class,int.class);method.setAccessible(true);Set<Long> ids=new HashSet<>();for(int cell=100;cell<103;cell++)for(int y=61;y<65;y++)for(int h=0;h<=16;h++)for(int rise=-1;rise<=1;rise++)for(int run:new int[]{0,1,24})for(int wet:new int[]{0,1,96})require(ids.add((Long)method.invoke(null,cell,y,h,rise,run,wet)),"state collision");});
