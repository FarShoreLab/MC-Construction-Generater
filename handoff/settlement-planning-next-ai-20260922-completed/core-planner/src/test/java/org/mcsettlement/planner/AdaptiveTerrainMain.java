...
public final class AdaptiveTerrainMain {
    ...
    public static void main(String[] args)throws Exception{
        ...
        run("all_registered_diagonal_preset_doors_and_whole_footprints",AdaptiveTerrainMain::diagonalDoors);
    ...
    private static void diagonalDoors(){for(var preset:BuildingPresetRegistry.getInstance().getAllPresets())for(String f:List.of("NORTH","EAST","SOUTH","WEST")){
        var shape=new BuildingShape(preset.rotateToFacing(f),true);var grid=shape.grid(null);check(shape.cells.size()>=preset.footprintArea()*.85,"Footprint cropped");check(shape.cells.size()<=preset.footprintArea()*1.15,"Unexpected raster area");
