        ...
        try:
            try:
                ...
                def fetch(url, options=None):
                    if not (url=='/api/plan' or url.startswith('/api/plan?') or url.startswith('/api/terrain?') or url=='/api/terrain-controls'):raise ValueError('Unexpected endpoint')
                    options=options or {}; body=options.get('body'); req=urllib.request.Request(a.base+url,data=body.encode() if body else None,headers=options.get('headers') or {},method=options.get('method','GET'))
                ...
                html=html.replace('<script>\n  const $','<script>window.fetch=async(url,options={})=>{const r=await window.__localFetch(url,{method:options.method,headers:options.headers,body:options.body});return {ok:r.status===200,status:r.status,json:async()=>JSON.parse(r.text)};};</script><script>\n  const $')
            ...
            page.select_option('#layoutMode','legacy')
