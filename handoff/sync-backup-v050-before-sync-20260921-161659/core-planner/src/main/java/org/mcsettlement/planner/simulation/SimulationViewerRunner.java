...
/**
 * Runner that executes the terrain block generator and bounded settlement algorithm,
...
public class SimulationViewerRunner {
    ...
    public static void main(String[] args) {
        ...
        for (String arg : args) {
            ...
            switch (pair[0]) {
                case "terrainType" -> terrainType = pair[1];
                case "terrainSeed", "seed" -> terrainSeed = Long.parseLong(pair[1]);
                case "planSeed" -> planSeed = Long.parseLong(pair[1]);
                case "targetPlots" -> targetPlots = Integer.parseInt(pair[1]);
                default -> { }
            }
        }

        System.out.printf("1. Generating 3D Minecraft voxel world [%dx%d, type: %s, relief: %d blocks, terrainSeed: %d]...\n",
                width, depth, terrainType, relief, terrainSeed);
        PipelineResult res = SimulatedSettlementPipeline.run(
                width, depth, baseElevation, relief, terrainType, terrainSeed, planSeed, targetPlots);
        System.out.printf(" - Planning with planSeed: %d; original terrain hash: %s%n", planSeed, res.originalTerrainHash);

        System.out.printf(" - Plan status: %s; allocated %d/%d; unmet groups: %d%n",
                res.plan.status, res.plan.plots.size(), targetPlots, res.plan.unmetRequirements.size());
        for (var unmet : res.plan.unmetRequirements) System.out.printf("   %s: %d/%d (%s)%n",
                unmet.requirementId, unmet.allocated, unmet.requested, unmet.reason);

        // Count only public road overlaps; private access intentionally crosses its own doorway.
        int collisions = 0;
        for (Plot p : res.plan.plots) {
            java.util.Set<Long> mask=new java.util.HashSet<>();
            for(int[] c:p.footprint)mask.add(org.mcsettlement.planner.RoadGeometry.key(p.origin2D[0]+c[0],p.origin2D[1]+c[1]));
            for(var column:res.plan.groundColumns)if("road".equals(column.kind)&&mask.contains(org.mcsettlement.planner.RoadGeometry.key(column.x,column.z)))collisions++;
        }

        System.out.println("2. Shared Construction Manifest & Overlap Check:");
        System.out.printf(" - Road Network Edges:     %d (paved-cell adjacency graph)\n", res.plan.transportNetwork.edges.size());
        ...
        System.out.printf(" - Road-Plot Collisions:   %d (manifest overlap count)\n", collisions);
        ...
        System.out.printf(" - Retaining Wall Blocks:  %d blocks (not generated by this planner)\n", res.retainingWallBlocks);
        System.out.printf(" - Public Road Columns:    %d blocks\n", res.roadBlocks);
        System.out.printf(" - Slope Stairs Placed:    %d blocks\n", res.stairBlocks);
        System.out.printf(" - Solid Edits (all kinds): %d blocks\n\n", res.buildingBlocks);

        // Print a vertical cross-section through the first planned house
        if (!res.plan.plots.isEmpty()) {
            Plot p1 = res.plan.plots.get(0);
            int midZ = p1.origin2D[1] + p1.builder.footprintSize[1]/2;
        ...
        File htmlFile = new File(outDir, "simulation_3d_viewer_legacy.html");
