...
final class TerrainRoadRouter {
    ...
    private static long id(int cell,int y,int heading,int rise,int run){return ((((long)cell*4096+y+2032)*13+heading)*3+rise+1)*25+run;}
    ...
    static Result routeTo(HeightfieldMap map,PlanRequest req,Set<Long> forbidden,Set<Long> roads,
        ...
        Node root=new Node(rootId,cell,start.y,heading,0,0,0,3*(guide==null?distances[cell]:guide.heuristic(cell,distances[cell])),null);best.put(rootId,root);open.add(root);stats.peakPathStates=Math.max(stats.peakPathStates,1);stats.peakOpenNodes=Math.max(stats.peakOpenNodes,1);
        int rejected=0;
        while(!open.isEmpty()&&stats.pathExpanded<end&&stats.pathExpanded<stats.pathLimit){
            ...
            for(int di=0;di<dirs.length;di++){
                ...
                for(int rise:new int[]{0,-1,1}){
                    ...
                    Node value=new Node(nextId,next,y,di,rise,run,g,g+3*(guide==null?distances[next]:guide.heuristic(next,distances[next])),n);best.put(nextId,value);open.add(value);
