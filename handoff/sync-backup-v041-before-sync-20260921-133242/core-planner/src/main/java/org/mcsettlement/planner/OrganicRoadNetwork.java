...
final class OrganicRoadNetwork {
    ...
    private void fallback(String reason) {
        NetworkMetrics m=ir.transportNetwork.metrics;m.status="FEASIBILITY_FALLBACK";m.reasons.addAll(failures);m.reasons.add(reason);
    ...
    private boolean connect(Hub from,Hub to,Plot plot,String role,boolean redundant,int remaining) {
        ...
        int maxTries=redundant?2:3;
        for(int attempt=0;attempt<maxTries;attempt++) {
            if(stats.topologyEdgesTried>=128||stats.pathExpanded>=stats.pathLimit||stats.gradeRelaxations>=stats.gradeLimit)return false;
            stats.topologyEdgesTried++;
            // Exclude existing roads away from junction regions: branches/loops cannot be duplicate paths.
            Set<Long> blocked=new HashSet<>(forbidden);boolean separate=redundant||attempt<2;
            if(separate)for(long k:network.roads) {
                if(near(k,from,7)||near(k,to,7))continue;
                for(int dx=-1;dx<=1;dx++)for(int dz=-1;dz<=1;dz++)blocked.add(key(x(k)+dx,z(k)+dz));
            }
            OrganicGuide guide=attempt==2?null:new OrganicGuide(map,from.x,from.z,to.x,to.z,
                    request.seed^SpatialNoise.mix(role.hashCode()),attempt,redundant?request.roadWidth*4+8:4);
            int allowance=Math.max(256,(stats.pathLimit-stats.pathExpanded)/Math.max(1,remaining));
            if(attempt==2&&distance(from,to)>64)allowance*=3;
            allowance=Math.min(30000,allowance);int end=Math.min(stats.pathLimit,stats.pathExpanded+allowance);
            TerrainRoadRouter.Result result=TerrainRoadRouter.routeTo(map,request,blocked,network.roads,fixed,
                    access,accesses,routes,entry,distances,stats,end,goals,guide);
            ...
            if(redundant) {
                ...
                if(fresh<request.roadWidth*10||enclosures(map,result.roadCells(),request.roadWidth)<=oldHoles) {
                    reason("REDUNDANCY_REJECTED_NO_NEW_PAVED_LOOP: "+from.id+" -> "+to.id);continue;
                }
            }
            Map<Long,GroundColumn> cols=combined(result.walk());
            if(cols.size()>stats.columnLimit){exhausted(stats,"GROUND_COLUMNS");continue;}
            if(editCount(cols)>stats.editLimit){exhausted(stats,"CONSTRUCTION_EDITS");continue;}
            RoadEdge edge=new RoadEdge();edge.id="organic_"+(network.routes.size()+1);edge.fromNodeId=from.id;edge.toNodeId=to.id;
            edge.roadType=role;edge.width=request.roadWidth;edge.steps=result.centers();edge.routingStyle=guide==null?"constrained_fallback":"sparse_arc_height_astar";
            if(guide!=null)edge.guideControls=guide.controls;
        ...
        reason("ROUTE_FAILED: "+role+" "+from.id+" -> "+to.id+" from="+from.x+","+from.y+","+from.z+" to="+to.x+","+to.y+","+to.z);return false;
    }
    ...
    private void commit(List<Hub> hubs,Hub root) {
        ...
        net.algorithm="clustered_plazas_mst_redundant_arcs";net.directionCount=request.roadDirections;net.corridorWidth=request.roadWidth;
        ...
        NetworkMetrics m=net.metrics;m.hubs=hubs.size();m.semanticNodes=net.semanticNodes.size();m.semanticEdges=net.semanticLinks.size();m.components=1;
