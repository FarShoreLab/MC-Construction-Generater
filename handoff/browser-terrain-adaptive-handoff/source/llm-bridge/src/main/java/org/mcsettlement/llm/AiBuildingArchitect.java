...
public class AiBuildingArchitect {
    ...
    public static BuildingVoxelModel designBuildingForPlot(
        ...
        // Planned settlements never make per-building LLM calls or re-resolve global prompt keywords.
        if ("locked_preset".equals(plot.builder.generatorType)) {
            String[][][] grid = org.mcsettlement.planner.preset.PlannedBuilding.grid(plot, settlementTheme);
            BuildingVoxelModel model = new BuildingVoxelModel();
            model.buildingName = plot.builder.presetId;
            model.sizeX = grid.length; model.sizeY = grid[0].length; model.sizeZ = grid[0][0].length;
            model.blocks = grid;
            return model;
        }
    ...
    public static BuildingVoxelModel synthesizeThematicBuilding(
        ...
        try {
            ...
            if (preset != null) {
                ...
                if (rotated.sizeX > width || rotated.sizeZ > depth || rotated.sizeY > height)
                    throw new IllegalArgumentException("Preset does not fit legacy plot; use the bounded planner");
