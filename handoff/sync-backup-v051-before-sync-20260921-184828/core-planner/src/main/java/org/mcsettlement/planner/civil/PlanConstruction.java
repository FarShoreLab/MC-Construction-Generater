    ...
    public static List<Edit> prepare(PlanningIR ir,String theme){
        ...
        for(GroundColumn c:ir.groundColumns){
            ...
            if(RoadTerrain.raised(c)) {
                if(!PlanningIR.EXPERT_SCHEMA_VERSION.equals(ir.metadata.version)||ir.sitePlanning==null||c.support!=(Math.floorMod(c.x+c.z,4)==0)||c.waterY==null||c.waterY<=c.originalY||c.targetY!=c.waterY+1||c.targetY-c.originalY>RoadTerrain.MAX_PILE_HEIGHT||c.facing!=null||"deck".equals(c.structure)!= "foundation".equals(c.kind))throw new IllegalArgumentException("INVALID_WET_DECK_COLUMN");
