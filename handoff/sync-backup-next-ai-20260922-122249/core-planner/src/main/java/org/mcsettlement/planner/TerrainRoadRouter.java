...
final class TerrainRoadRouter {
    ...
    private static long id(int cell,int y,int heading,int rise,int run,int wetRun){return (((((long)cell*4096+y+2032)*17+heading)*3+rise+1)*25+run)*97+wetRun;}
    ...
    static Result routeTo(HeightfieldMap map,PlanRequest req,Set<Long> forbidden,Set<Long> roads,
        ...
        Node root=new Node(rootId,cell,start.y,startHeading,startRise,0,startWet,0,heuristicWeight*(guide==null?distances[cell]:guide.heuristic(cell,distances[cell])),null);best.put(rootId,root);open.add(root);stats.peakPathStates=Math.max(stats.peakPathStates,1);stats.peakOpenNodes=Math.max(stats.peakOpenNodes,1);
        ...
        while(!open.isEmpty()&&stats.pathExpanded<end&&stats.pathExpanded<stats.pathLimit){
            ...
            for(int di=0;di<dirs.length;di++){
                ...
                for(int rise:new int[]{0,-1,1}){
                    ...
                    int g=n.g+movement+nearby+soil+Math.abs(rise)*7+turn+(n.rise*rise<0?12:0)+(guide==null?0:guide.cost(next)+guide.straightCost(n.run,di==n.heading));
                    ...
                    Node value=new Node(nextId,next,y,di,rise,run,wetRun,g,g+heuristicWeight*(guide==null?distances[next]:guide.heuristic(next,distances[next])),n);best.put(nextId,value);open.add(value);
    ...
    private static int cost(HeightfieldMap m,PlanRequest r,Set<Long> forbidden,Map<Long,GroundColumn> fixed,
        ...
        // Price the approach and the span continuously: a fill-limit discontinuity makes
        // A* prefer a high deck over the short ramp needed to reach that same deck.
        int elevationCost=r.expert!=null&&r.expert.allowBridges&&!RoadTerrain.wet(m,x+dx,z+dz)?6:18;
        return (soil*6)/Math.max(1,brush.size())+Math.abs(RoadTerrain.natural(m,x+dx,z+dz)-ny)*elevationCost+(RoadTerrain.wet(m,x+dx,z+dz)?24:0);
    }
}
