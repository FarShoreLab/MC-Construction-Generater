...
def query_values(parsed, terrain_only: bool = False) -> dict[str, str]:
    defaults = {
        ...
        "roadSurface": "cobblestone", "presetPalette": "classic", "singlePresetId": "square_cabin",
    ...
    if defaults["presetPalette"] not in {"classic", "mixed", "compact", "spacious", "single", "crafted", "estates", "custom"}:
    ...
    if json.loads((RESOURCES / "presets" / (defaults["singlePresetId"] + ".json")).read_text(encoding="utf-8")).get("archived") and defaults["presetPalette"] == "single":
        raise ValueError("PRESET_ARCHIVED")
    selection=json.loads(defaults["buildingSelection"],object_pairs_hook=unique_object)
    if not isinstance(selection,list) or len(selection)>64: raise ValueError("BUILDING_SELECTION_LIMIT")
    if defaults["settlementMode"] not in {"village","town","city"}:
        raise ValueError("Invalid settlementMode")
    if defaults["siteSeed"] != "follow":
        site_seed=int(defaults["siteSeed"]);
        if not -9007199254740991 <= site_seed <= 9007199254740991: raise ValueError("siteSeed outside safe integer range")
        defaults["siteSeed"]=str(site_seed)
    for key in ["allowBridges", "autoDock"]:
        if defaults[key] not in {"true", "false"}:
            raise ValueError(key + " must be true or false")
    for key, high in [("minBBoxCoverage", .85), ("minMinorAxisRatio", .9), ("buildingRepulsion", 3)]:
...
class Handler(BaseHTTPRequestHandler):
    server_version = "MCSettlementPreview/5.1"
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            for key,value in obj.items():
                if key in {"pins", "buildingSelection"}:
