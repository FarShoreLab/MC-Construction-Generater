...
with sync_playwright() as pw:
 ...
 try:
  ...
  def generate(name,size,terrain,relief,palette,seed=42,directions=8,diagonal=False,depth=None):
   page.select_option('#mapSize','custom' if depth is not None else str(size))
   if depth is not None:page.fill('#mapWidth',str(size));page.fill('#mapDepth',str(depth))
   page.select_option('#terrainType',terrain);page.fill('#relief',str(relief));page.fill('#baseElevation','58');page.fill('#terrainSeed','42');page.fill('#planSeed',str(seed));page.fill('#targetPlots','7');page.select_option('#roadDirections',str(directions));page.set_checked('#diagonalBuildings',diagonal);page.select_option('#presetPalette',palette)
   before=time.perf_counter();page.click('#generate');page.wait_for_function(f'state.current.config.width==={size} && state.current.config.depth==={depth or size} && state.current.config.terrainType==="{terrain}" && state.current.config.relief==={relief} && state.current.config.presetPalette==="{palette}" && state.current.config.planSeed==={seed} && state.current.config.roadDirections==={directions} && !state.requestPending',timeout=95000)
  ...
  generate('reproduction512',512,'rolling_hills',24,'mixed',45,12)
  assert results['scenes']['reproduction512']['metrics']['network']['maximumStraightRun']<=16
  ...
  generate('custom192x96',192,'rolling_hills',24,'classic',43,12,True,96)
